...
class Memory(nn.Module):
    ...
    def forward(self, features, coarse_pred, mask):
        """
...
class Memorynet(nn.Module):
    ...
    def forward(self, pos, x, object_labels, y=None, epoch_ratio=None):
        ...
        x3 = self.memory1(x3, coarse_pred1, mask_float)
        ...
        x2 = self.memory2(x2, coarse_pred2, mask_float)
...
def train_loop(dataloader, model, loss_fn, metric_fn, optimizer, device, 
    ...
    for i, (pos, x, y, object_labels) in enumerate(pbar):
        ...
        y_pred, coarse_seg_loss = model(pos, x, object_labels, y, cur_epoch/total_epoch)
        loss = loss_fn(y_pred, y) + + coarse_seg_loss
...
def val_loop(dataloader, model, loss_fn, optimizer, lr_scheduler, device, cur_epoch, path, show_gap, log_dir):
    ...
    with torch.no_grad():
        for pos, x, y, object_labels in dataloader:
            ...
            y_pred, _ = model(pos, x, object_labels)
    ...
    with torch.no_grad():
        for pos, x, y, object_labels in tqdm(dataloader):
            ...
            for i in range(voting_num):
                ...
                temp_pred, _ = model(temp_pos, temp_x, object_labels)
                y_pred += temp_pred
...
if __name__ == '__main__':
    ...
    log_dir = 'partseg/memorynet_partseg.log'
    ...
    epochs = 200
    show_gap = 1
    save_path = 'partseg/memorynet_partseg.pth'
