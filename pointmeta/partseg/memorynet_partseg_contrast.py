...
class PointSetAbstractionLayer(nn.Module):
    def __init__(self, stride, radius, k, in_channels, mlp_units):
        ...
        x_mlp = [nn.Conv1d(in_channels, mlp_units[0], kernel_size=1),
                    nn.BatchNorm1d(mlp_units[0]),
                    nn.ReLU(inplace=True)]
        for i in range(len(mlp_units) - 2):
            x_mlp += [nn.Conv1d(mlp_units[i], mlp_units[i + 1], kernel_size=1),
                    nn.BatchNorm1d(mlp_units[i + 1]),
                    nn.ReLU(inplace=True)]
        x_mlp += [nn.Conv1d(mlp_units[-2], mlp_units[-1], kernel_size=1),
                    nn.BatchNorm1d(mlp_units[-1])]

        self.x_mlp = nn.Sequential(*x_mlp)
        
        self.pos_mlp = nn.Sequential(nn.Conv2d(3, 32, kernel_size=1),
                                     nn.BatchNorm2d(32),
                                     nn.ReLU(True),
                                     nn.Conv2d(32, 32, kernel_size=1),
                                     nn.BatchNorm2d(32),
                                     nn.ReLU(True),
                                     nn.Conv2d(32, mlp_units[-1], kernel_size=1),
                                     nn.BatchNorm2d(mlp_units[-1]),
                                     nn.ReLU(True))
        self.skip_mlp = nn.Linear(in_channels, mlp_units[-1])
    ...
    def forward(self, points, features, gts=None):
        ...
        cen_features = index_points(features, fps_indices)
        if gts is not None:
        ...
        group_features = F.relu(group_features.transpose(1, 2) + self.skip_mlp(cen_features), True)
...
class Memorynet(nn.Module):
    def __init__(self, num_class):
        ...
        self.in_linear = nn.Linear(7, 160)
        self.sa1 = PointSetAbstractionLayer(2, 0.1, 32, 160, [160, 160, 320])
        self.sa2 = PointSetAbstractionLayer(2, 0.1*2.5, 32, 320, [320, 320, 640])
        self.sa3 = PointSetAbstractionLayer(2, 0.1*2.5*2.5, 32, 640, [640, 640, 1280])
        self.sa4 = PointSetAbstractionLayer(2, 0.1*2.5*2.5*2.5, 32, 1280, [1280, 1280, 2560])
        
        self.global_conv1 = nn.Sequential(nn.Linear(2560, 128),
                                          nn.ReLU(True))
        self.global_conv2 = nn.Sequential(nn.Linear(1280, 64),
                                          nn.ReLU(True))

        self.fp1 = PointFeaturePropagationLayer(2560+1280, [1280, 1280])
        self.pe_gen1 = PEGenerator(1280)
        self.coarse_mlp1 = nn.Sequential(nn.Conv1d(1280, 1280, kernel_size=1, bias=False),
                                        nn.BatchNorm1d(1280),
                                        nn.ReLU(inplace=True),
                                        nn.Conv1d(1280, num_class, kernel_size=1))
        self.coarse_loss_fn1 = PolyFocalLoss(2.0, 0.25, 1.0)
        self.memory1 = Memory(num_class, 32, 1280, 1280)
        
        self.fp2 = PointFeaturePropagationLayer(1280+640, [640, 640])
        self.pe_gen2 = PEGenerator(640)
        self.coarse_mlp2 = nn.Sequential(nn.Conv1d(640, 640, kernel_size=1, bias=False),
                                        nn.BatchNorm1d(640),
                                        nn.ReLU(inplace=True),
                                        nn.Conv1d(640, num_class, kernel_size=1))
        self.coarse_loss_fn2 = PolyFocalLoss(2.0, 0.25, 1.0)
        self.memory2 = Memory(num_class, 32, 640, 640)
        
        self.fp3 = PointFeaturePropagationLayer(640+320, [320, 320])
        self.pe_gen3 = PEGenerator(320)
        self.coarse_mlp3 = nn.Sequential(nn.Conv1d(320, 320, kernel_size=1, bias=False),
                                        nn.BatchNorm1d(320),
                                        nn.ReLU(inplace=True),
                                        nn.Conv1d(320, num_class, kernel_size=1))
        self.coarse_loss_fn3 = PolyFocalLoss(2.0, 0.25, 1.0)
        self.memory3 = Memory(num_class, 32, 320, 320)
        
        self.fp4 = PointFeaturePropagationLayer(320+128+64+160+16, [160, 160])

        self.mlp = nn.Sequential(nn.Conv1d(480, 480, kernel_size=1),
                                    nn.BatchNorm1d(480),
                                    nn.ReLU(inplace=True),
                                    nn.Dropout(0.5),
    ...
    def forward(self, pos, x, object_labels, y=None, epoch_ratio=None):
        ...
        global_x4 = self.global_conv1(x4).max(dim=1, keepdim=True)[0].expand(-1, n, -1)   # shape = (b, n, 128)
        global_x3 = self.global_conv2(x3).max(dim=1, keepdim=True)[0].expand(-1, n, -1)

        x3 = self.fp1(pos3, x3, pos4, x4)
        # 给x3加上pe
        x3 = self.pe_gen1(pos3, x3, 0.1*2.5*2.5*2.5, 32)
        coarse_pred1 = self.coarse_mlp1(x3.transpose(1, 2))
        coarse_seg_loss1 = 0
        if self.training:
            coarse_seg_loss1 = self.coarse_loss_fn1(coarse_pred1, y3)
            self.memory1.update(x3, y3, coarse_pred1, epoch_ratio)
        x3, _ = self.memory1(x3, coarse_pred1, mask_float)
        
        x2 = self.fp2(pos2, x2, pos3, x3)
        # 给x2加上pe
        x2 = self.pe_gen2(pos2, x2, 0.1*2.5*2.5, 32)
        coarse_pred2 = self.coarse_mlp2(x2.transpose(1, 2))
        coarse_seg_loss2 = 0
        if self.training:
            coarse_seg_loss2 = self.coarse_loss_fn2(coarse_pred2, y2)
            self.memory2.update(x2, y2, coarse_pred2, epoch_ratio)
        x2, _ = self.memory2(x2, coarse_pred2, mask_float)
        
        x1 = self.fp3(pos1, x1, pos2, x2)
        # 给x1加上pe
        x1 = self.pe_gen3(pos1, x1, 0.1*2.5, 32)
        coarse_pred3 = self.coarse_mlp3(x1.transpose(1, 2))
        coarse_seg_loss3 = 0
        if self.training:
            coarse_seg_loss3 = self.coarse_loss_fn3(coarse_pred3, y1)
            self.memory3.update(x1, y1, coarse_pred3, epoch_ratio)
        x1, contrast_loss = self.memory3(x1, coarse_pred3, mask_float, y1)
        
        object_labels = F.one_hot(object_labels, 16).to(dtype=torch.float32)
        object_labels = object_labels.unsqueeze(dim=1).expand(-1, n, -1)
        x = self.fp4(pos, torch.cat((x, global_x4, global_x3, object_labels), dim=-1), pos1, x1)
        ...
        coarse_seg_loss = (coarse_seg_loss1 + coarse_seg_loss2 + coarse_seg_loss3) * (0.4 / 3)
...
object_to_part_onehot = torch.zeros((16, 50), dtype=torch.uint8, device='cuda:7')
...
if __name__ == '__main__':
    ...
    log_dir = 'partseg/memorynet_partseg_contrast_x3x2x1.log'
    ...
    epochs = 200
    show_gap = 1
    save_path = 'partseg/memorynet_partseg_contrast_x3x2x1.pth'
