...
class Memorynet(nn.Module):
    def __init__(self, num_class, stride, k, InvRes_num_list):
        ...
        self.coarse_mlp3 = nn.Sequential(nn.Conv1d(128, 128, kernel_size=1, bias=False),
                                        nn.BatchNorm1d(128),
                                        nn.ReLU(inplace=True),
    ...
    def forward(self, pos, x, y=None, epoch_ratio=None):
        ...
        # 给x1加上pe
        x1 = self.pe_gen3(pos1, x1, 0.2, 32)
        coarse_pred3 = self.coarse_mlp3(x1.transpose(1, 2))
        coarse_seg_loss3 = 0
        if self.training:
            coarse_seg_loss3 = F.cross_entropy(coarse_pred3, y1, label_smoothing=0.2)
            self.memory3.update(x1, y1, coarse_pred3, epoch_ratio)
        x1, contrast_loss = self.memory3(x1, coarse_pred3, y1)
        
        x = self.fp4(pos, x, pos1, x1)
        
        y_pred = self.out_mlp(x.transpose(1, 2))
        coarse_seg_loss = coarse_seg_loss1 * 0.1 + coarse_seg_loss2 * 0.1 + coarse_seg_loss3 * 0.1
...
if __name__ == '__main__':
    ...
    log_dir = f'seg/logs/memorynet_seg_4_contrast_area5_{seed}.log'
