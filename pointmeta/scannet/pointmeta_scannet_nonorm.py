...
from model_nonorm import PointMeta
...
if __name__ == '__main__':
    ...
    log_dir = 'scannet/logs/pointmeta_scannet_nonorm.log'
    # logging.basicConfig(filename=log_dir, format='%(message)s', level=logging.INFO)
    if rank == 0:
        with open(log_dir, mode='a') as f:
            f.write(f'random seed {seed}\n')

    train_aug = Compose([PointCloudRotation_Z(1.0, False),
    ...
    train_dataset = Scannet_NoNorm('/mnt/Disk16T/chenhr/threed_data/data/ScanNet', split='train', loop=6, npoints=64000, transforms=train_aug)
    val_aug = Compose([ColorNormalize(mean=[0.46259782, 0.46253258, 0.46253258], std=[0.693565, 0.6852543, 0.68061745])])
    val_dataset = Scannet_NoNorm('/mnt/Disk16T/chenhr/threed_data/data/ScanNet', split='val', loop=1, npoints=None, transforms=val_aug)
    ...
    loss_fn = nn.CrossEntropyLoss(label_smoothing=0.2, ignore_index=-100)
    
    # 配置不同的weight decay
    parameter_group = get_parameter_groups(pointmeta, weight_decay=1e-4, log_dir=log_dir, rank=rank)
    ...
    epochs = 100
    show_gap = 1
    save_path = 'scannet/checkpoints/pointmeta_scannet_nonorm.pth'
