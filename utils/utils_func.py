...
class SemanticAwareAttention_allepsc(nn.Module):
    def __init__(self, embed_dim, kdim, vdim):
        super(SemanticAwareAttention_allepsc, self).__init__()
        self.embed_dim = embed_dim
    ...
    def forward(self, q, k, v, length, coarse_pred=None, need_weights=False):
        """
        q.shape = (b, n, embed_dim)
        k.shape = (b, num_class * length, kdim)
        v.shape = (b, num_class * length, vdim)
        coarse_pred.shape = (b, num_class, n)
        """
        q = self.q_proj(q)
        k = self.k_proj(k)
        ...
        if coarse_pred is not None:
            coarse_pred = coarse_pred.detach()
            coarse_pred = coarse_pred.transpose(1, 2).softmax(dim=-1)
            coarse_pred = coarse_pred.unsqueeze(dim=-1).expand(-1, -1, -1, length).flatten(-2, -1)
            attn_map = torch.softmax(attn_map * coarse_pred, dim=-1)
            # attn_map = attn_map * coarse_pred
            # attn_map = attn_map / attn_map.sum(dim=-1, keepdim=True)
...
class PEGenerator_Meta(nn.Module):
    def __init__(self, out_channel):
        super(PEGenerator_Meta, self).__init__()
        self.pe_mlp = nn.Sequential(nn.Conv2d(3, out_channel, 1, bias=False),
    ...
    def forward(self, pos, x, radius, k):
        ...
        group_pos, _, _ = ball_query_cuda2(radius, k, pos, pos, pos)   # (b, 3, k, n)
        group_pos = group_pos - pos.transpose(1, 2).unsqueeze(dim=2)
        pe, _ = self.pe_mlp(group_pos / radius).max(dim=2)
        x = x + pe.transpose(1, 2)
        
        return x


class PEGenerator_vanilla(nn.Module):
    def __init__(self, out_channel):
        super(PEGenerator_vanilla, self).__init__()
        self.pe_mlp = nn.Linear(3, out_channel)
