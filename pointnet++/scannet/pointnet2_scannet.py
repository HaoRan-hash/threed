...
class Pointnet2(nn.Module):
    ...
    def forward(self, pos, x, y=None):
        ...
        x = torch.cat((pos, x, height), dim=-1)
        ...
        coarse_seg_loss1 = 0
        if self.training:
            coarse_seg_loss1 = F.cross_entropy(coarse_pred1, y3, label_smoothing=0.1)
            
        x2 = self.fp2(pos2, x2, pos3, x3)
        coarse_pred2 = self.coarse_mlp2(x2.transpose(1, 2))
        coarse_seg_loss2 = 0
        if self.training:
            coarse_seg_loss2 = F.cross_entropy(coarse_pred2, y2, label_smoothing=0.1)
...
def train_loop(dataloader, model, loss_fn, optimizer, device, 
    ...
    for i, (pos, x, y) in enumerate(pbar):
        ...
        with torch.cuda.amp.autocast():
            y_pred, coarse_seg_loss = model(pos, x, y)
    ...
    with torch.no_grad():
        for pos, x, y, sort_idx, counts in pbar:
            ...
            for i in range(counts.max()):
                ...
                cur_pred, _ = model(cur_pos, cur_x)
    ...
    with torch.no_grad():
        for pos, x, y, sort_idx, counts in pbar:
            ...
            for i in range(counts.max()):
                ...
                cur_pred, _ = model(multi_scale_pos, multi_scale_x)
...
if __name__ == '__main__':
    ...
    log_dir = 'scannet/pointnet2_scannet.log'
    # logging.basicConfig(filename=log_dir, format='%(message)s', level=logging.INFO)
    # if rank == 0:
    #     with open(log_dir, mode='a') as f:
    #         f.write(f'random seed {seed}\n')

    # train_aug = Compose([PointCloudRotation_Z(1.0),
    #                      PointCloudScaling(0.8, 1.2, mirror=[0.2, -1, -1]),
    #                      ColorContrast(p=0.2),
    #                      ColorDrop(p=0.2),
    #                      ColorNormalize(mean=[0.46259782, 0.46253258, 0.46253258], std=[0.693565, 0.6852543, 0.68061745])])
    # train_dataset = Scannet('/home/lindi/chenhr/threed/data/ScanNet', split='train', loop=6, npoints=64000, transforms=train_aug)
    # val_aug = Compose([ColorNormalize(mean=[0.46259782, 0.46253258, 0.46253258], std=[0.693565, 0.6852543, 0.68061745])])
    # val_dataset = Scannet('/home/lindi/chenhr/threed/data/ScanNet', split='val', loop=1, npoints=None, transforms=val_aug)

    # train_sampler = DistributedSampler(train_dataset, shuffle=True)
    # val_sampler = DistributedSampler(val_dataset, shuffle=False)

    # train_dataloader = DataLoader(train_dataset, batch_size=8, num_workers=8, sampler=train_sampler)
    ...
    ddp_pointnet2 = nn.parallel.DistributedDataParallel(pointnet2, device_ids=[rank], output_device=rank)
    loss_fn = nn.CrossEntropyLoss(label_smoothing=0.1)
    
    # 配置不同的weight decay
    parameter_group = get_parameter_groups(ddp_pointnet2, weight_decay=1e-4, log_dir=log_dir, rank=rank)
    ...
    # epochs = 100
    # show_gap = 1
    save_path = 'scannet/pointnet2_scannet.pth'
    # for i in range(epochs):
    #     train_loop(train_dataloader, ddp_pointnet2, loss_fn, optimizer, device, i, epochs, show_gap, 1, rank)
    #     val_loop(val_dataloader, ddp_pointnet2, loss_fn, optimizer, lr_scheduler, device, i, save_path, show_gap, log_dir, rank)
    #     lr_scheduler.step()
    
    # test entire room
    test_aug = Compose([ColorNormalizeTensor(mean=[0.46259782, 0.46253258, 0.46253258], std=[0.693565, 0.6852543, 0.68061745])])
    test_dataset = Scannet('/home/lindi/chenhr/threed/data/ScanNet', split='test', loop=1)
    test_sampler = DistributedSampler(test_dataset, shuffle=False)
    test_dataloader = DataLoader(test_dataset, batch_size=1, num_workers=8, sampler=test_sampler)
    test_entire_room(test_dataloader, test_aug, ddp_pointnet2, loss_fn, device, save_path, log_dir, rank)
    multi_scale_test(test_dataloader, test_aug, ddp_pointnet2, loss_fn, device, save_path, log_dir, rank)
