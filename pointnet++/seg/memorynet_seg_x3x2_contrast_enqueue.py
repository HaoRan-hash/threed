        ...
        for i in range(self.num_class):
            ...
            if n != 0 :   # 如果存在该类的feature
                ...
                if self.cur_occupy[i] != self.length:   # 该类的memory未满
                    ...
                    update_length = self.length // 4
                    choice = choice[0:update_length]
                    cur_features = cur_features[choice]
                    self.memory[i, self.cur_update[i]:self.cur_update[i]+update_length] = cur_features * cur_miu + self.memory[i, self.cur_update[i]:self.cur_update[i]+update_length] * (1 - cur_miu)
                    self.cur_update[i] = (self.cur_update[i] + update_length) % self.length
...
if __name__ == '__main__':
    ...
    log_dir = 'seg/logs/memorynet_seg_x3x2_contrast_enqueue.log'
    ...
    epochs = 100
    show_gap = 1
    save_path = 'seg/checkpoints/memorynet_seg_x3x2_contrast_enqueue.pth'
