...
from utils_func import ball_query_cuda2, knn_query_cuda2, index_points, index_gts, SemanticAwareAttention_allepsc, PEGenerator
...
class Memory(nn.Module):
    def __init__(self, num_class, length, store_channels, query_channels, base_miu=0.1, end_miu=0.001):
        ...
        self.attention = SemanticAwareAttention_allepsc(query_channels, store_channels, store_channels)
    ...
    def forward(self, features, coarse_pred, gts=None):
        ...
        memory_features2 = self.memory.flatten(0, 1).unsqueeze(dim=0).expand(b, -1, -1)
        memory_features = F.normalize(memory_features, dim=-1)
        memory_features2 = F.normalize(memory_features2, dim=-1)
        if gts is not None:
        ...
        reve_features = self.attention(F.normalize(features, dim=-1), memory_features2, memory_features2, coarse_pred)
...
if __name__ == '__main__':
    ...
    log_dir = 'seg/logs/memorynet_seg_x3x2_contrast_all_epsc.log'
    ...
    epochs = 100
    show_gap = 1
    save_path = 'seg/checkpoints/memorynet_seg_x3x2_contrast_all_epsc.pth'
