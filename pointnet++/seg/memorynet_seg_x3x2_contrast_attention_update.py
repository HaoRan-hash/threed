        ...
        for i in range(self.num_class):
            ...
            if n != 0 :   # 如果存在该类的feature
                ...
                if self.cur_occupy[i] != self.length:   # 该类的memory未满
                    ...
                    attn_map = torch.mm(self.memory[i], cur_features.T) / math.sqrt(self.store_channels)
                    merge_features = torch.mm(attn_map.softmax(dim=-1), cur_features)
                    self.memory[i] = merge_features * cur_miu + self.memory[i] * (1 - cur_miu)
...
if __name__ == '__main__':
    ...
    log_dir = 'seg/logs/memorynet_seg_x3x2_contrast_attention_update.log'
    ...
    epochs = 100
    show_gap = 1
    save_path = 'seg/checkpoints/memorynet_seg_x3x2_contrast_attention_update.pth'
