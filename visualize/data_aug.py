...
class PointCloudRotation:
    ...
    def __call__(self, pos, x):
        angle = np.random.uniform(low=(-self.angle), high=self.angle, size=(3,))
        angle_cos = np.cos(angle)
        angle_sin = np.sin(angle)
        
        rotation_matrix_x = np.array([[1, 0, 0],
                                      [0, angle_cos[0], -angle_sin[0]],
                                      [0, angle_sin[0], angle_cos[0]]])
        rotation_matrix_y = np.array([[angle_cos[1], 0, -angle_sin[1]],
                                      [0, 1, 0],
                                      [angle_sin[1], 0, angle_cos[1]]])
        rotation_matrix_z = np.array([[angle_cos[2], -angle_sin[2], 0],
                                      [angle_sin[2], angle_cos[2], 0],
                                      [0, 0, 1]])
