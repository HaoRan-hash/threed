...
class PointSetAbstractionLayer(nn.Module):
    def __init__(self, stride, radius, k, in_channels, mlp_units, is_group_all=False):
        ...
        for i in range(len(mlp_units) - 2):
            mlp += [nn.Conv2d(mlp_units[i], mlp_units[i + 1], kernel_size=1),
                    ...
                    nn.ReLU(inplace=True)]
        mlp += [nn.Conv2d(mlp_units[-2], mlp_units[-1], kernel_size=1),
                    nn.BatchNorm2d(mlp_units[-1])]

        self.mlp = nn.Sequential(*mlp)
        self.skip_mlp = nn.Linear(in_channels-3, mlp_units[-1])
    ...
    def group_all(self, points, features):
        ...
        centroids = self.index_points(points, choice)   # centroids.shape = (b, 1, 3)
...
class Memorynet(nn.Module):
    def __init__(self, num_class):
        ...
        self.sa1 = PointSetAbstractionLayer(2, 0.1, 32, 160+3, [160, 160, 320])
        self.sa2 = PointSetAbstractionLayer(2, 0.1*2.5, 32, 320+3, [320, 320, 640])
        self.sa3 = PointSetAbstractionLayer(2, 0.1*2.5*2.5, 32, 640+3, [640, 640, 1280])
        self.sa4 = PointSetAbstractionLayer(2, 0.1*2.5*2.5*2.5, 32, 1280+3, [1280, 1280, 2560])
    ...
    with torch.no_grad():
        for pos, x, y, object_labels in tqdm(dataloader):
            ...
            for i in range(len(y_pred)):
                ...
                cur_y = y[i]
                # with open(log_dir, mode='a') as f:
                #     f.write(f'{cur_y_pred.unique(return_counts=True)},   {cur_y.unique(return_counts=True)}\n')
