...
def train_loop(dataloader, model, loss_fn, metric_fn, optimizer, device, 
    ...
    for i, (pos, x, y, object_labels) in enumerate(pbar):
        ...
        with torch.cuda.amp.autocast():
            y_pred = model(pos, x, object_labels)
    ...
    with torch.no_grad():
        for pos, x, y, object_labels in tqdm(dataloader):
            ...
            for i in range(voting_num):
                ...
                temp_pred = model(temp_pos, temp_x, object_labels)
                y_pred += temp_pred
...
if __name__ == '__main__':
    ...
    log_dir = 'partseg/pointnext_partseg_adam.log'
    ...
    pointnext = Pointnext(50).to(device)
    ...
    optimizer = torch.optim.Adam(pointnext.parameters(), lr=1e-3)
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [140, 180], 0.1)

    epochs = 200
    show_gap = 1
    save_path = 'partseg/pointnext_partseg_adam.pth'
    for i in range(epochs):
        train_loop(train_dataloader, pointnext, loss_fn, metric_acc, optimizer, device, i, epochs, show_gap, 1)
        val_loop(val_dataloader, pointnext, loss_fn, optimizer, lr_scheduler, device, i, save_path, show_gap, log_dir)
        lr_scheduler.step()
    ...
    voting_test(test_dataloader, pointnext, loss_fn, device, save_path, log_dir, 10)
